  ...
  return (
    <div className="p-6">
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          { label: 'Total Sent', value: stats.total_sent, sub: `${stats.total_campaigns} campaigns`, color: 'text-blue-600' },
          { label: 'Open Rate', value: `${stats.open_rate}%`, sub: `${stats.total_opens} opens`, color: 'text-green-600' },
          { label: 'Click Rate', value: `${stats.click_rate}%`, sub: `${stats.total_clicks} clicks`, color: 'text-purple-600' },
          { label: 'Bounce Rate', value: `${stats.bounce_rate}%`, sub: `${stats.total_bounced} bounced`, color: 'text-red-600' },
        ].map((s) => (
      ...
      <div className="mt-6 rounded-xl bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 grid grid-cols-5 gap-3 text-center text-sm">
          <div><p className="text-2xl font-bold text-gray-900">{contactStats.total}</p><span className="text-gray-500">Total</span></div>
          <div><p className="text-2xl font-bold text-green-600">{contactStats.active}</p><span className="text-gray-500">Active</span></div>
          <div><p className="text-2xl font-bold text-red-600">{contactStats.bounced}</p><span className="text-gray-500">Bounced</span></div>
          <div><p className="text-2xl font-bold text-orange-600">{contactStats.complained}</p><span className="text-gray-500">Complained</span></div>
          <div><p className="text-2xl font-bold text-gray-500">{contactStats.unsubscribed}</p><span className="text-gray-500">Unsubscribed</span></div>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {recentCampaigns.length === 0 ? (
              ...
              <tr key={c.id} className="cursor-pointer hover:bg-gray-50" onClick={() => navigate(`/campaigns/${c.id}`)}>
                ...
                <td className="px-3 py-2">{c.sent_count}/{c.total_recipients}</td>
